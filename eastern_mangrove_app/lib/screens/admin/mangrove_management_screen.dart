...
class _MangroveManagementScreenState extends State<MangroveManagementScreen>
          ...
          child: Column(
            ...
            children: [
              ...
              if (area['mangrove_species'] != null)
                _buildDetailRow('พันธุ์ไม้', area['mangrove_species'] is List ? (area['mangrove_species'] as List).join(', ') : area['mangrove_species'].toString()),
              ...
              if (area['threats'] != null && area['threats'].toString().isNotEmpty) ...[
                ...
                const SizedBox(height: 4),
                Text(area['threats'] is List ? (area['threats'] as List).join(', ') : area['threats'].toString()),
              ],
              if (area['conservation_activities'] != null && area['conservation_activities'].toString().isNotEmpty) ...[
                ...
                const SizedBox(height: 4),
                Text(area['conservation_activities'] is List ? (area['conservation_activities'] as List).join(', ') : area['conservation_activities'].toString()),
              ],
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (selectedLat == null || selectedLon == null) {
                  setInnerState(() {
                    dialogError = '⚠️ กรุณาเลือกตำแหน่งพื้นที่ — กดการ์ด "เลือกตำแหน่งบนแผนที่" หรือ "ใช้ตำแหน่งปัจจุบัน"';
                  });
                  return;
                }
  ...
  Widget _buildMapTab() {
    // Calculate center - use filtered areas with location or default to Thailand center
    final areasWithLocation = _filteredAreas.where((a) => a['latitude'] != null && a['longitude'] != null).toList();
    ...
    return Column(
      children: [
        ...
        if (_filteredAreas.isNotEmpty && areasWithLocation.isEmpty)
          Positioned(
            top: 16,
            left: 16,
            right: 16,
